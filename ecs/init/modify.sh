...
docker exec -it wordpress sed -i "s/.*DB_PASSWORD.*/define('DB_PASSWORD', '$2');/g" /etc/wordpress/config.php
docker exec -it wordpress sed -i "s/.*DB_HOST.*/define('DB_HOST', '$1');/g" /etc/wordpress/config.php
