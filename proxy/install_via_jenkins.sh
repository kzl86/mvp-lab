...
echo ' ***  Add user to the system, use authorized keys of the existing centos user *** '
adduser $USER
mkdir /home/$USER/.ssh/
chown $USER:$USER /home/$USER/.ssh/
cp /home/centos/.ssh/authorized_keys /home/$USER/.ssh/authorized_keys
chown $USER:$USER /home/$USER/.ssh/authorized_keys
chmod 600 /home/$USER/.ssh/authorized_keys

echo ' ***  Copy the user and the group ids to separate variables *** '
USER_ID=$(cat /etc/passwd | grep $USER | awk -F: '{print $3}')
USER_GID=$(cat /etc/passwd | grep $USER | awk -F: '{print $4}')

echo ' *** Creating the shared folder, set permissions *** '
mkdir -p /share/uploads
chmod 755 /share 
chown $USER:$USER /share/uploads

echo ' *** Chroot the sftp user, restart service to take effect. *** '
...
echo ' *** Installing NFS server, enabling and starting services *** '
...
echo ' *** Register the shared folder to the NFS exports file. This share will trust only one IP address *** '
...
echo ' *** Add NFS rules to firewall *** '
...
echo ' *** Configure reverse proxy, enable and start service *** '
...
echo ' *** SELinux enable httpd processes to initiate outbound connection *** '
...
echo ' *** Add firewall rule to enable incoming 80/tcp and reload. *** '
...
echo ' *** Create configuration, ssh clients will be banned for 24 hour after 3 unsuccessfull attempt *** '
